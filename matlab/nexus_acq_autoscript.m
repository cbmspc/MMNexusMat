...
NumRecordedSessions = length(aa);
LastRecordedSession = 0;
for i = 1:NumRecordedSessions
    tmp = regexp(aa(i).name, '-(\d+)nexus','tokens','once');
    if ~isempty(tmp) && str2double(tmp{1}) > LastRecordedSession
        LastRecordedSession = str2double(tmp{1});
    end
end


fprintf('There are already %i recorded sessions. The last session number is %i\n', NumRecordedSessions, LastRecordedSession);
...
while retry
    ...
    naa_in3 = 0;
    SessionNum = num2str(naa_in3,'%.2i');
    if isempty(naa_in3) || ~isnumeric(naa_in3) || round(naa_in3) ~= naa_in3 || naa_in3 < 0 || isinf(naa_in3) || naa_in3 > 99
        fprintf('Session Number must be integer between 1 and 99 (inclusive)\n');
        retry = 1;
        continue
    elseif exist([getdesktopdir() filesep SubjectName '-' SessionNum 'nexus' NexusNum '.mat'], 'file')
        fprintf('Session already recorded. The last recorded session was %02i\n', LastRecordedSession);
        retry = 1;
        continue
    elseif naa_in3 ~= 0 && naa_in3 ~= LastRecordedSession+1
        fprintf('Warning: You seem to have skipped a session number. \n');
        fprintf('         This should be session %02i but you entered %02i\n', LastRecordedSession+1, naa_in3);
...
thand(2) = text(0.0, 0.9, ['Subject: ' SubjectName 10 '  Nexus: ' NexusNumDisp ' (' num2str(length(ChanNames)) ' chans, ' num2str(NexusAcqSampleRate) ' Hz)' 10 'Session: ' SessionNum 10 '   Date: ' datestr(now,'yyyy-mm-dd HH:MM')], 'FontSize', 16, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontName', 'FixedWidth');
...
LocalSaveFile = [getdesktopdir() filesep SubjectName '-' SessionNum 'nexus' NexusNum '.mat'];
...
RemoteSaveFile = [RemoteSavePath filesep SubjectName '-' SessionNum 'nexus' NexusNum '.mat'];
